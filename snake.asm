...
fin: .asciiz " point seulement t'est vraiment nul meme moi je suis meilleur \n"
...
majDirection:
#calcul de l'opposer de snakeDir
li $t0,4	
addi $t1,$a0,2		#ajoute 2 a la valeur de la position demandée
move $t2,$zero  	#remet a zero le registre t2
div $t1,$t0		#divise par 4 la position demandée +2
mfhi $t2		#met le reste dans le registre t2

#verification de la direction
lw $t3,snakeDir 	
beq $t3,$a0,finMaj	#si la direction du snake = la direction demandée -> finMaj
beq $t3,$t2,finMaj	#si la direction du snake = la direction opposé -> finMaj
sw $a0,snakeDir		#rangement de la valeur de direction dans snakeDir
...
updateGameStatus:
move $s1 $ra
lw $a1 tailleSnake #initialisation de valeurs
li $t0 1
j deplacementSerpent

deplacementSerpent: #fonction de déplacement du corp du serpent
blt $a1 $0 deplacement #sortie de la boucle quand tout le serpent a été déplacé
sll $t0 $a1 2 #shift de l'adresse mémoire
li $t1 4
add $t1 $t0 $t1 #adresse mémoire de la nouvelle position
lw $t2 snakePosX($t0)
sw $t2 snakePosX($t1)
lw $t2 snakePosY($t0)
sw $t2 snakePosY($t1) #enregistre la position du segment du serpent à la nouvelle position
subi $a1 $a1 1
j deplacementSerpent #répétition de la fonction

deplacement: #deplace le serpent d'une case dans la direction de la tete du serpent
lw $t0 snakeDir
li $t1 0
beq $t0 $t1 deplacementHaut
li $t1 1
beq $t0 $t1 deplacementDroite
li $t1 2
beq $t0 $t1 deplacementBas
li $t1 3
beq $t0 $t1 deplacementGauche
j Bouffe

deplacementHaut: #deplace la tete du serpent de 1 vers le haut
lw $t0 snakePosX
addi $t0 $t0 1
sw $t0 snakePosX
j Bouffe

deplacementBas: #deplace la tete du serpent de 1 vers le bas
lw $t0 snakePosX
subi $t0 $t0 1
sw $t0 snakePosX
j Bouffe

deplacementDroite: #deplace la tete du serpent de 1 vers la droite
lw $t0 snakePosY
addi $t0 $t0 1
sw $t0 snakePosY
j Bouffe

deplacementGauche: #deplace la tete du serpent de 1 vers la gauche
lw $t0 snakePosY
subi $t0 $t0 1
sw $t0 snakePosY
j Bouffe

Bouffe: #tente de manger le bonbon
lw $t0 snakePosX
lw $t1 candy
lw $t3 snakePosY
li $t4 4
lw $t4 candy($t4)
bne $t0 $t1 updateGameStatusFin 
bne $t3 $t4 updateGameStatusFin #si la tete n'est pas sur le bonbon alors ne le mange pas

#si la tete est sur le bonbon
lw $t8 scoreJeu 
lw $t5 tailleSnake
addi $t5 $t5 1
addi $t8 $t8 1
sw $t5 tailleSnake
sw $t8 scoreJeu #augmente le score et la taille du serpent      

jal newRandomObjectPosition
lw $t0 numObstacles
addi $t0 $t0 1
sw $t0 numObstacles #créé un nouvel obstacle
sll $t0 $t0 2
sw $v0 obstaclesPosX($t0) 
sw $v1 obstaclesPosY($t0) #enregistre la position du nouvel obstacle

jal newRandomObjectPosition #créé un nouveau bonbon
sw $v0 candy
li $t0 4
sw $v1 candy($t0) #écrase la position de l'ancien bonbon avec la position du nouveau bonbon
j updateGameStatusFin
...
li $a3 0
lw $t0, snakePosX  # Coordonnée X de la tête du serpent
lw $t1, snakePosY  # Coordonnée Y de la tête du serpent

li $t2, 16  # Taille de la grille (nombre de cases dans une dimension)

# Vérifier si la tête du serpent est en dehors des limites de la grille
blt $t0, $zero, finJeu
blt $t1, $zero, finJeu
bge $t0, $t2, finJeu
bge $t1, $t2, finJeu
...
lw $t3, numObstacles  # Nombre d'obstacles
li $t4, 0
li $t7, 0

checkObstacleCollisionLoop:
bge $t4, $t3, verifQueue  # Si tous les obstacles ont été vérifiés, continuer le jeu

lw $t5, obstaclesPosX($t7)  # Coordonnée X de l'obstacle
lw $t6, obstaclesPosY($t7)  # Coordonnée Y de l'obstacle

# Vérifier si la tête du serpent est en collision avec un obstacle
beq $t0, $t5, checkYCollision
j incrementIndices

checkYCollision:
beq $t1, $t6, finJeu  # Collision avec l'obstacle, fin du jeu
j incrementIndices

incrementIndices:
addi $t4, $t4, 1
addi $t7, $t7, 4
j checkObstacleCollisionLoop

verifQueue:
blt $a1, 3, continueGame  # Si la taille du serpent est <= 0, continuer le jeu
j checkYCollisionQueue

checkYCollisionQueue:
beq $a3, $a2, continueGame  # Si tous les éléments de la queue ont été vérifiés, continuer le jeu
lw $t5, snakePosX($t8)
lw $t6, snakePosY($t8)

# Vérifier si la tête du serpent est en collision avec un élément de la queue
beq $t0, $t5, checkXCollisionQueue
j incrementQueueIndex

checkXCollisionQueue:
beq $t1, $t6, finJeu  # Collision avec un élément de la queue, fin du jeu
j incrementQueueIndex
...
continueGame:
li $v0, 0  # Continuer le jeu
j finConditionFinJeu
